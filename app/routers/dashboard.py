...
async def dashboard(
    ...
    recent_txs, _ = await tx_svc.get_transactions(db, user.id, page=1, per_page=10)
    ...
    prev_url = f"/dashboard?period={period}&ref={prev_ref.isoformat()}"
    next_url = f"/dashboard?period={period}&ref={next_ref.isoformat()}"
    base_url = f"/dashboard?ref={ref_date.isoformat()}"
