...
### Fixed
- **Sessions no longer die silently at 30 minutes.** The refresh token was consumed on its first use after the access token lapsed but nothing issued a new pair, so exactly one request survived the 30-minute mark and every one after it bounced to `/login` &mdash; a fixed timer from sign-in, not an idle timeout, and invisible until the next round trip. Presenting the refresh token now rotates the pair (a 60-second reuse grace covers requests already in flight with the old cookie; a token revoked outright by logout or a password change gets none), so a session that is being used renews itself and idles out only after `REFRESH_TOKEN_EXPIRE_DAYS`. Cookie lifetimes follow the settings instead of hardcoded values
- When a session really has lapsed it says so and loses nothing: navigations go to `/login?next=&hellip;&expired=1` (same-origin paths only) and the login page shows a notice and returns the user to that page; a lapsed POST returns to the page it came from rather than replaying. htmx/fetch traffic gets a **401** with `HX-Redirect` instead of a 302 &mdash; the browser followed that silently and handed scripts the login page as a 200 &mdash; and a global fetch guard in the shell turns it into a toast and a trip to the login page instead of a handler quietly choking on login-page HTML
- Reconciliation ticks survive a lapse: the tick set is mirrored to `localStorage` per statement on every change and restored (with a notice) if a Save Draft or Finish never landed; Cancel and Discard draft clear it. An activity-gated keep-alive (`/auth/ping` every ten minutes while the tab is in use) renews the session during long ticking sessions and surfaces a dead one within minutes rather than at Save
